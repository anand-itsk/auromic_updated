                                    ...
                                    <div class="form-group row">
                                            ...
                                            <select class="form-control select2" name="company_type_id"
                                                ...
                                                @foreach ($company_types as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ $delivery_challans->company->company_type_id == $item->id ? 'selected' : '' }}>
                                                        {{ $item->name }}</option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select class="form-control select2 w-100" name="company_id" id="company_id">
                                                ...
                                                @foreach ($company as $companyItem)
                                                    <option value="{{ $companyItem->id }}"
                                                        {{ $delivery_challans->company_id == $companyItem->id ? 'selected' : '' }}>
                                        ...
                                        <div class="col-sm-4 mb-4">
                                            <input class="form-control" type="text" name="sub_company_name"
                                                id="sub_company_name" value="{{ $subCompanyName }}">
                                            @error('dc_date')
                                                <span class="error" style="color: red;">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="col-sm-4 mb-4">
                                            <input class="form-control" type="date" name="dc_date" id="dc_date"
                                                value="{{ $delivery_challans->dc_date }}">
                                            @error('dc_date')
                                                <span class="error" style="color: red;">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <label for="customer_name" class="col-sm-2 col-form-label">DC
                                            Number</label>
                                        <div class="col-sm-4 mb-4">
                                            <input class="form-control" type="text" name="dc_number"
                                                id="dc_number"value="{{ $delivery_challans->dc_no }}" readonly>
                                            @error('dc_number')
                                                <span class="error" style="color: red;">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="col-sm-4 mb-4">
                                            <select class="form-control select2" name="order_id" id="order_id">
                                                ...
                                                @foreach ($order_nos as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ $item->id == $delivery_challans->order_id ? 'selected' : '' }}>
                                                        {{ $item->customer_order_no }}</option>
                                                @endforeach
                                            </select>
                                            @error('order_id')
                                                <span class="error" style="color: red;">{{ $message }}</span>
                                        ...
                                        <div class="col-sm-4 mb-4">
                                            <input class="form-control" type="text" name="quantity"
                                                id="quantity"value="{{ $delivery_challans->quantity }}">
                                            @error('quantity')
                                                <span class="error" style="color: red;">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="col-sm-4 mb-4">
                                            <input class="form-control" type="text" name="weight" id="weight"
                                                value="{{ $delivery_challans->weight }}">
                ...
                $.ajax({
                    ...
                    success: function(company) {
                        $('#company_id').empty().append(
                            '<option value="">Select Company</option>');
                        $('#company_id').append('<option value="' + company.id + '" selected>' +
        ...
        $(document).ready(function() {
            $('#order_id').change(function() {
                ...
                if (orderId) {
                            ...
                            if (response.length > 0) {
                                $.each(response, function(key, value) {
                                    options += '<option value="' + value.id + '">' +
                                        value.model_name + ' - ' + value.model_code + ' (Qty: ' + value.quantity + ')'+
                                        '</option>';
                                });
                                $('#product_model').html(options).prop('disabled', false);
                                ...
                                $('#product_model').html(options).prop('disabled', true);
                            }
                        },
                        error: function() {
                    ...
                    $('#product_model').html('<option value="">Select Model</option>').prop('disabled',
                        true);
                }
            });
                ...
                if (productModelId) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $('#product_size_id').val(response.product_size.code);
                        },
                        error: function(xhr, status, error) {
                            console.error(error);
                        }
