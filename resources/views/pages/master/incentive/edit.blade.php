...
<div class="col-sm-4 mb-4">
   <select class="form-control" name="product_id" id="product">
    ...
    @foreach($products as $product)
        <option value="{{ $product->id }}" @if($incentive->product->id == $product->id) selected @endif>{{ $product->name }}</option>
    ...
    @foreach($productModels as $productModel)
        <option value="{{ $productModel->id }}" data-product-id="{{ $productModel->product_id }}" 
    ...
    @endforeach
                           ...
                           @enderror
                                    ...
                                    <div class="col-sm-4 mb-4">
                                        <input class="form-control" type="text" name="model_size" id="model_size"value="{{$incentive->model_size}}">
                                        @error('model_size')
                                        <span class="error" style="color: red;">{{ $message }}</span>
                                        @enderror
                                    </div>
  ...
  <div class="col-sm-4 mb-4">
    <input class="form-control" type="text" name="duration_period" id="duration_period" value="{{$incentive->duration_period}}">
    ...
    <div class="form-group">
        <div class="d-flex justify-content-evenly">
            ...
            <a href="{{ route('master.incentives.index') }}" class="btn btn-secondary waves-effect m-l-5">
                Cancel
            </a>
        </div>
    </div>
